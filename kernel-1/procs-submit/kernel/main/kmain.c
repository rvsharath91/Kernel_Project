...
int faberTest(kshell_t *kshell, int argc, char **argv)
{
    ...
    temp_proc = proc_create("faber_test()");
    dbg(DBG_PRINT, "(GRADING1C): faber_test() is invoked \n");
    temp_thread = kthread_create(temp_proc,faber_thread_test,0,NULL);
...
int sunghanTest(kshell_t *kshell, int argc, char **argv)
{
    ...
    proc_t *temp_proc;
    kthread_t *temp_thread;
    temp_proc = proc_create("sunghan test()");
    dbg(DBG_PRINT, "(GRADING1D 1): sunghan test() is invoked\n");
    temp_thread = kthread_create(temp_proc,sunghan_test,0,NULL);
    sched_make_runnable(temp_thread);/*Make the thread runnable*/
    do_waitpid(temp_proc->p_pid,0,&status); /*waiting for the test to end*/
    return 0;
}
