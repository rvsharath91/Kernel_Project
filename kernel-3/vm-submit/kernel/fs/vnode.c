...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
    ...
    dbg(DBG_PRINT,"(GRADING2A 1.a) file is either a character device or block device\n");


    if(S_ISBLK(file->vn_mode))
    {
        dbg(DBG_ERROR, "(special_file_read): Returning ENOTSUP as it is a block device \n" );
        dbg(DBG_PRINT, "(GRADING2B) Returning ENOTSUP as it is a block device  \n" );
        return -ENOTSUP;
    }
    if (S_ISCHR(file->vn_mode))
    {
        KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
        dbg(DBG_PRINT,"(GRADING2A 1.a) We can perform read operation on the corresponding file\n");
    }
...
/*if(S_ISCHR(file->vn_mode))*/
    if (S_ISCHR(file->vn_mode))
    {
        ...
        if(file->vn_cdev->cd_ops->read)
        {
            dbg(DBG_PRINT, " (GRADING2B)Returning the output of read as it is a character device\n" );
            return file->vn_cdev->cd_ops->read(file->vn_cdev,offset,buf,count);
        }
        /*else 
        {
            dbg(DBG_ERROR, "(special_file_read): Function not implemented \n" );
            dbg(DBG_PRINT, "(GRADING2B) Function not implemented\n" );
            return -ENOSYS;
        }*/
    }
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
    ...
    KASSERT(file);
    dbg(DBG_PRINT, "(GRADING2A 1.b) file exists\n");

    KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
    dbg(DBG_PRINT,"(GRADING2A 1.b) file is either a character device or block device\n");


    if(S_ISBLK(file->vn_mode))
    {
        dbg(DBG_ERROR, "(special_file_write): Returning ENOTSUP as it is a block device \n" );
        dbg(DBG_PRINT, "(GRADING2B) Returning ENOTSUP as it is a block device  \n" );
        return -ENOTSUP;
    }
    if (S_ISCHR(file->vn_mode))
        ...
        dbg(DBG_PRINT,"(GRADING2A 1.b) we can perform read operation on the corresponding file\n");
        }

    if (S_ISCHR(file->vn_mode))
    {
        if(file->vn_cdev->cd_ops->write)
        {
            dbg(DBG_PRINT, "(GRADING2B) Returning the output of write as it is a character device\n" );
