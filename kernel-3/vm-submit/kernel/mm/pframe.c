...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
    ...
    {
         dbg(DBG_VM,"grade95\n");
           dbg(DBG_PRINT,"(GRADING3B 1),pframe returned is NULL\n"); 
          /* if(pageoutd_needed())
           {  
           dbg(DBG_VM,"grade96\n");  
            dbg(DBG_PRINT,"(GRADING3B 1),waking u pageoutd\n"); 
                pageoutd_wakeup();
            }*/

      res=pframe_alloc(o,pagenum);
      if (res == NULL) {
        dbg(DBG_VM,"grade97\n");
        dbg(DBG_PRINT,"(GRADING3D 3),No memory to allocate pframe\n");
                return -ENOMEM;
            }
      int ret_val =  pframe_fill(res); 
      *result = res;
      KASSERT(NULL!=*result);
      dbg(DBG_PRINT,"(GRADING3A 1.a),pframe is valid\n");
      KASSERT(!pframe_is_busy(*result));
      dbg(DBG_PRINT,"(GRADING3A 1.a),pframe is not busy\n");
      ...
      return ret_val;
        ...
        {
            dbg(DBG_VM,"grade98\n");
            dbg(DBG_PRINT,"(GRADING3B 1),pframe returned is not NULL\n"); 
        while(pframe_is_busy(res))
        {
            dbg(DBG_VM,"grade99\n");
          dbg(DBG_PRINT,"(GRADING3D),pframe is busy calling sched_sleep\n");   
        sched_sleep_on(&res->pf_waitq);
        }
    }
      *result = res;
dbg(DBG_VM,"grade100\n");
...
pframe_pin(pframe_t *pf)
{
    ...
    dbg(DBG_PRINT,"(GRADING3A 1.b),pframe is not free\n");       
    KASSERT(pf->pf_pincount>= 0);
    dbg(DBG_PRINT,"(GRADING3A 1.b),pincount for pframe is valid\n");
    if (!pframe_is_pinned(pf)){
    dbg(DBG_VM,"grade101\n");
    dbg(DBG_PRINT,"(GRADING3B 1),pframe is not pinned, pinning now\n");
        list_remove(&pf->pf_link);
...
{
    KASSERT(!(pframe_is_free(pf)));
    dbg(DBG_PRINT,"(GRADING3A 1.c),pframe is not free\n");       
    KASSERT(pf->pf_pincount> 0);
    dbg(DBG_PRINT,"(GRADING3A 1.c),pincount for pframe is valid\n");
    pf->pf_pincount--;

    if (pf->pf_pincount == 0)
    {
        dbg(DBG_VM,"grade102\n");
        dbg(DBG_PRINT,"(GRADING3B 1),pincount is 0, unpinning pframe\n");
        list_remove(&pf->pf_link);
