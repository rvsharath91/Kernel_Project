...
{
    vmarea_t *vma;
    uint32_t phy = ADDR_TO_PN(vaddr);

    vma = vmmap_lookup(p->p_vmmap,phy);

    if((vma!=NULL) && (vma->vma_prot & perm))
    {
        dbg(DBG_VM,"grade03\n");
        dbg(DBG_PRINT,"(GRADING3D 1),The process has valid permission on vaddr\n");
            return 1;
            
    }
    dbg(DBG_PRINT,"(GRADING3D 1),The process has valid permission for the range\n");
    dbg(DBG_VM,"grade04\n");
        return 0;
}
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
    ...
    uint32_t start = ADDR_TO_PN(avaddr);
    uint32_t end = ADDR_TO_PN((uint32_t)avaddr + (len - 1)); 
    uint32_t temp = start;
    int  ret_val;
    void* virt;
    while( temp <= end) 
    {
    virt = PN_TO_ADDR(temp); 
    ret_val= addr_perm(p,virt,perm);
     if (!ret_val) {
        dbg(DBG_VM,"grade01\n");
    dbg(DBG_PRINT,"(GRADING3D 1),The process does not have valid permission for the range\n");
     return 0;
    } 
    temp++;
  }
