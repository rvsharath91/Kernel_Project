...
sys_read(read_args_t *arg)
{
    ...
    void * buf = page_alloc();
     ...
     copy_from_user(&targ, arg, sizeof(write_args_t)) ;

     len = MIN(PAGE_SIZE,targ.nbytes);

     err =  do_read(targ.fd,buf,len);

      if (err < 0)
        { 
            dbg(DBG_VM,"grade05\n");
            dbg(DBG_PRINT,"(GRADING3D 1),error from do_read\n");
            curthr->kt_errno = -err;
                page_free(buf);
                return -1;
        }

    copy_to_user(targ.buf,buf,len);

    page_free(buf); 
dbg(DBG_VM,"grade06\n");
    dbg(DBG_PRINT,"(GRADING3D 1),sys_read was successfull\n"); 
    return err;
}
...
sys_write(write_args_t *arg)
{
    ...
    {    
    dbg(DBG_VM,"grade07\n");       
        dbg(DBG_PRINT,"(GRADING3D 1),characters to be read > 0\n"); 
    copy_from_user(buf,targ.buf,len);

    err =  do_write(targ.fd,buf,len);          
  
   if (err < 0)
        { 
            dbg(DBG_VM,"grade08\n");
            dbg(DBG_PRINT,"(GRADING3D 1),error from do_write\n");
            curthr->kt_errno = -err;
                page_free(buf);
                return -1;
         }
    ...
    page_free(buf);  
dbg(DBG_VM,"grade09\n");
    dbg(DBG_PRINT,"(GRADING3D 1),sys_write was successfull\n"); 
    return err;
}
...
sys_getdents(getdents_args_t *arg)
{
      /*  NOT_YET_IMPLEMENTED("VM: sys_getdents");
        return -1;*/
     getdents_args_t targ;
     int err=0,dent=0,dented=0;
     copy_from_user(&targ, arg, sizeof(getdents_args_t)); 
     int countt = (int)(targ.count/sizeof(dirent_t));
     dirent_t temp;
     char* buf;
      size_t  size_len = 0;
     while(countt>0)
     {
        err = do_getdent(targ.fd, &temp);
        if(err< 0)
        {
            dbg(DBG_VM,"grade010\n");
            dbg(DBG_PRINT,"(GRADING3D 1),error returned from do_getdent\n");
            curthr->kt_errno = -err;
            return -1;
        }
        else if(err>0)
        {
            dbg(DBG_VM,"grade11\n");
            dbg(DBG_PRINT,"(GRADING3D 1),sys_getdents will continue\n");
            size_len = dented * sizeof(dirent_t);
            buf = (char*)(targ.dirp) + size_len;
            copy_to_user(buf, &temp, sizeof(dirent_t));
            countt--;
            dented++;
            dent = dent + err;     
        }
        ...
        {
            dbg(DBG_VM,"grade12\n");
            dbg(DBG_PRINT,"(GRADING3D 1),sys_getdents was completed\n");
            countt = -1;
        }
       
     }
     dbg(DBG_VM,"grade13\n");
     dbg(DBG_PRINT,"(GRADING3D 1),sys_getdents was successfull\n"); 
     return dent;
}
