...
printf("The client 2 sends the request %s to the Server 1 with port number %d and IP address %s\n",k_v[y],(int) ntohs(server.sin_port),inet_ntoa(server.sin_addr));
...
printf("The Client 2 received the value %s from the Server 1 with port number %d and IP address %s\n",b,(int) ntohs(server.sin_port), inet_ntoa(server.sin_addr));
printf("The Client2's port number is %d and IP address is %s\n\n",(int) htons(cl.sin_port), inet_ntoa(cl.sin_addr));
